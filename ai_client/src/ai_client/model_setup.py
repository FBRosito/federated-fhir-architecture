...
if _BASE_PRECISION not in {"nf4", "fp16", "bf16"}:
    log.warning("Unknown MODEL_BASE_PRECISION=%r — falling back to 'nf4'", _BASE_PRECISION)
    _BASE_PRECISION = "nf4"
...
def load_quantized_model(
    ...
    _kwargs: dict = dict(device_map=device_map, token=hf_token, attn_implementation="eager")

    if _BASE_PRECISION == "nf4":
        quant_cfg = quant_cfg or QuantizationConfig()
        bnb_config = build_bnb_config(quant_cfg)
        log.info("Loading model — NF4 4-bit (BitsAndBytes double quant): %s", model_name)
        _kwargs["quantization_config"] = bnb_config
    else:
        dtype = torch.float16 if _BASE_PRECISION == "fp16" else torch.bfloat16
        log.info("Loading model — %s full precision (no quantisation, VRAM ~2× NF4): %s",
                 _BASE_PRECISION.upper(), model_name)
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained(model_name, local_files_only=True, **_kwargs)

    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total     = sum(p.numel() for p in model.parameters())
    log.info("Base parameters — total: %d M | trainable before LoRA: %d | precision: %s",
             total // 1_000_000, trainable, _BASE_PRECISION)
...
def apply_lora(
    ...
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    log.info("Trainable LoRA parameters: %d (%.4f%% of total)",
             trainable,
...
class ClinicalICD10Dataset(Dataset):
    ...
    def __init__(
        ...
        log.info("Dataset created: %d examples, max_length=%d", len(self.items), max_length)
...
def train_one_round(
    ...
    if calibrating and _calib_norms:
        ...
        log.info(
            ...
            p50, p75, p95, len(_calib_norms),
        )
        log.info("RECOMMENDATION: use max_grad_norm=%.6f (75th percentile)", p75)
        import json, pathlib
        out = pathlib.Path(os.getenv("FL_CALIB_OUTPUT", "/tmp/grad_norm_calibration.json"))
...
def train_continuous(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step, batch in enumerate(dataloader):
            ...
            if accum_count % gradient_accum_steps == 0:
                ...
                if math.isfinite(float(total_norm)):
                    ...
                    log.warning("Non-finite grad norm at global_step %d — optimizer step skipped.", global_step)
                scaler.update()
                optimizer.zero_grad()
                accum_count = 0
                if global_step >= warmup_steps:
        ...
        metrics  = {
            "epoch":            epoch,
            "train_loss":       round(avg_loss, 6),
            "train_perplexity": round(float(torch.exp(torch.tensor(avg_loss)).item()), 4),
            "opt_steps_total":  global_step,
        }
        log.info(
            "Epoch %d/%d completed — loss=%.4f | ppl=%.2f | accumulated_opt_steps=%d",
            epoch, num_epochs, avg_loss, metrics["train_perplexity"], global_step,
        )
...
def set_lora_parameters(model: PreTrainedModel, parameters: list[np.ndarray]) -> None:
    ...
    log.info("LoRA weights updated with aggregated parameters from server (%d tensors).", len(keys))
