...
def _build_narrative(
    ...
    if vitals:
        vital_parts = []
        for itemid, (label, value, unit) in sorted(
            vitals.items(), key=lambda x: x[1][0]
        ):
            unit_str = f" {unit}" if unit and unit not in ("", "nan") else ""
...
def load_all_icd_codes(hosp_dir: Path, hadm_ids: set[int]) -> dict[int, list[str]]:
    ...
    log.info(
        "  All ICD-10 codes loaded: %d admissions, mean %.1f codes/admission.",
        len(result),
...
def load_base_data(
    ...
    # ── Main join ──
    df = (
        diag.merge(icd_dict, on="icd_code", how="left")
    ...
    for split_name in ("train", "val", "test"):
        log.info(
            "Split %s: %d admissions.",
            split_name,
            (df["temporal_split"] == split_name).sum(),
        )
    ...
    if benchmark in ("top50", "full"):
        ...
        if benchmark == "top50":
            ...
            log.info(
                "Benchmark top50: %d most frequent ICD-10 codes.", len(valid_codes)
            )
            ...
            log.info(
                "Benchmark full: %d ICD-10 codes with ≥10 occurrences.",
                len(valid_codes),
            )
            if label_index_output:
                import json as _json
        ...
        log.info(
            "After benchmark '%s' filter: %d → %d admissions.",
            benchmark,
            before,
            len(df),
        )
    ...
    if dirichlet_alpha > 0.0:
        log.info("Dirichlet partitioning: α=%.2f | %d silos", dirichlet_alpha, n_silos)
    ...
    selected["all_icd_codes"] = (
        selected["hadm_id"]
        .astype(int)
        .map(lambda hid: ",".join(all_codes_map.get(hid, [])))
    )
...
def load_labs(
    hosp_dir: Path, hadm_ids: set[int]
...
def load_medications(
    hosp_dir: Path, hadm_ids: set[int], max_per_admission: int = 15
) -> dict[int, list[str]]:
    """
...
def write_fhir_bundles(
    ...
    for i, (_, row) in enumerate(base.iterrows()):
        series = _build_row_series(
            row, labs, microbiology, procedures, vitals, notes, medications, full_notes
        )
...
def main() -> None:
    """CLI entry point: assemble FHIR bundles from MIMIC-IV CSV exports."""
    ...
    if using_notes:
        ...
        log.info(
            "Mode: synthetic narratives (MIMIC-IV-Note not available at %s).", note_dir
        )
    ...
    if using_notes:
        ...
        full_notes = load_notes(
            note_dir, hadm_candidates, max_chars=999_999, skip_leakage_filter=True
        )
        ...
        dist_str = " | ".join(
            f"silo {pid}: {(base['partition_id'] == pid).sum()}"
            for pid in range(n_silos)
        )
