...
> **No Docker, no Java required.** Services run directly as Python processes.
> The FHIR R4 server is a lightweight Python stdlib implementation — no external dependencies.
...
- [x] PhysioNet account with MIMIC-IV credentialed access (physionet.org)
- [ ] HuggingFace token with Llama-3.2 access — *optional, only for Experiment B*
...
| `uv sync` + FHIR server start + ETL load | ~20 min | ~$0.35 |
| Mini-validation (Step 6.5) | ~15 min | ~$0.25 |
| `run_nodocker.sh --exp A` (Experiment A only) | ~10 h | ~$10 |
| **Total cloud (Exp A)** | **~12 h** | **~$12** |
...
For **Experiment A** (paper), no tokens are required. For **Experiment B** (extension), add your HuggingFace token:

```bash
nano .env   # Ctrl+O to save, Ctrl+X to exit
# Set HF_TOKEN=hf_YOUR_TOKEN_HERE  (only for Exp B Llama model)
```
...
## Step 6 — Start FHIR R4 server and load data

The `run_nodocker.sh` script handles this automatically at startup. To verify manually:

```bash
# Start the Python FHIR R4 in-memory server (no Java, no download needed):
nohup python3 fhir_server.py 8080 > experiment_logs/fhir_server.log 2>&1 &
...
| FHIR server not ready | Server crashed | `tail experiment_logs/fhir_server.log` |
| FL silos exit without metrics | FHIR server has no data | Re-run ETL: `FHIR_SERVER_URL=http://localhost:8080/fhir uv run etl-worker` |
...
| FHIR server loses data between runs | In-memory server restarted | `run_nodocker.sh` detects this and re-runs ETL automatically |
