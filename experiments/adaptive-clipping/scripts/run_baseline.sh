#!/usr/bin/env bash
set -euo pipefail
# run_baseline.sh <sigma> <seed>
#
# Global-clip baseline (ADAPTIVE_CLIPPING_STRATEGY=baseline): 1 adaptive-clipping
# FL server + 5 silos, PubMedBERT/ICD-10, 20 rounds.
#
# Assumes the FHIR R4 server + ETL data load are ALREADY running (via the
# root repo's `make up-infra` / `run_nodocker.sh`) — this script only starts
# the FL server + silos for this experiment, mirroring run_nodocker.sh's
# start_fl_server/_run_silo pattern but self-contained to this package.
...
TAG="baseline_sigma${SIGMA}_seed${SEED}"
