#!/usr/bin/env bash
set -euo pipefail
# run_sigma_sweep.sh <sigma> <seed>
#
# Sigma sweep at fixed C0=1.0: 1 adaptive-clipping FL server + 5 silos,
# PubMedBERT/ICD-10, 20 rounds. Reuses the exact same
# ADAPTIVE_CLIPPING_STRATEGY=baseline code path as run_baseline.sh — the
# only thing that varies here is FL_NOISE_MULTIPLIER (sigma); FL_MAX_GRAD_NORM
# is pinned to 1.0 across every point in the sweep. Maps the actual epsilon
# vs micro_f1 trade-off curve: sigma=1.0 (already in the baseline logs)
# gives micro_f1~0.105 (SNR=1/sigma=1.0 for the classifier head, the
# apparent bottleneck established by the C0 sweep and the corrected
# per_layer validation). This sweep covers sigma in {0.1, 0.3, 0.5, 2.0} —
# sigma=0.1 (SNR=10) is expected to show genuine learning at the cost of a
# much larger epsilon (~17416, RDPAccountant-verified); sigma=2.0 (SNR=0.5)
# is expected to be at least as flat as sigma=1.0, at a smaller epsilon (~6.35).
#
...
TAG="sigma_sweep_sigma${SIGMA}_seed${SEED}"
