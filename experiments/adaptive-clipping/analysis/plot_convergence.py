...
from analysis.bootstrap_ci import NUM_SILOS, SIGMAS, STRATEGIES, per_seed_value
...
# baseline.
_STYLE = {
    ("baseline", 1.0):  dict(color="#08519c", linestyle="--", label="Baseline (global clip), σ=1.0"),
    ("baseline", 2.0):  dict(color="#6baed6", linestyle="--", label="Baseline (global clip), σ=2.0"),
    ("per_layer", 1.0): dict(color="#d94801", linestyle="-",  label="Per-layer clip, σ=1.0"),
    ("per_layer", 2.0): dict(color="#fd8d3c", linestyle="-",  label="Per-layer clip, σ=2.0"),
}


def load_round_records(logs_root: Path, strategy: str, sigma: float) -> dict[int, dict[int, list[dict]]]:
    """Returns {round: {seed: [records per silo]}} for one (strategy, sigma) config."""
    by_round: dict[int, dict[int, list[dict]]] = {r: {} for r in range(1, NUM_ROUNDS + 1)}
...
def config_curve(logs_root: Path, strategy: str, sigma: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for round_idx in rounds:
        ...
        if seed_values:
            ...
            stds[round_idx - 1] = float(np.std(seed_values, ddof=1)) if len(seed_values) > 1 else 0.0
...
def plot_all_configs(logs_root: Path, output_dir: Path) -> None:
    ...
    for strategy in STRATEGIES:
        for sigma in SIGMAS:
            ...
            ax.fill_between(rounds, means - stds, means + stds, color=style["color"], alpha=0.15)

    ax.axhline(CENTRALIZED_F1, color="gray", linestyle=":", linewidth=1.5)
    ax.text(1.1, CENTRALIZED_F1 + 0.006, f"Centralized (F1={CENTRALIZED_F1})", fontsize=8, color="gray")
    ax.axhline(FL_NODP_F1, color="gray", linestyle=":", linewidth=1.5)
    ax.text(1.1, FL_NODP_F1 + 0.006, f"FL no DP (F1={FL_NODP_F1})", fontsize=8, color="gray")
