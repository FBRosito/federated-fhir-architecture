...
def _f1_std(logs_root: Path, strategy: str, sigma: float) -> float:
    ...
    seed_values = [
        per_seed_value(records, "micro_f1") for records in records_by_seed.values()
    ]
...
def build_table(ci_csv_path: Path, logs_root: Path) -> pd.DataFrame:
    """Build the results table DataFrame from the bootstrap-CI CSV plus paired tests."""
    ...
    for sigma in SIGMAS:
        ...
        comparisons[sigma] = wilcoxon_test(
    ...
    for sigma in SIGMAS:
        ...
        f1_by_strategy = {
            s: group[(group["strategy"] == s) & (group["metric"] == "micro_f1")].iloc[0]
            for s in ("baseline", "per_layer")
        }
        eps_by_strategy = {
            s: group[(group["strategy"] == s) & (group["metric"] == "epsilon")].iloc[0]
            for s in ("baseline", "per_layer")
        }
            ...
            rows.append(
                {
                    ...
                    "delta_vs_baseline": (
                        None if is_baseline else f1_row["mean"] - baseline_f1_mean
                    ),
                    "p_bonferroni": (
                        None
                        if is_baseline
                        else comparisons[sigma]["p_value_bonferroni"]
                    ),
                }
            )
    ...
    for sigma in SIGMAS:
        ...
        baseline_ci = ConfidenceInterval(
            mean=baseline_row["micro_f1_mean"],
            std=0.0,
            ci_lower=baseline_row["micro_f1_ci_lower"],
            ci_upper=baseline_row["micro_f1_ci_upper"],
            n=10,
        )
        candidates = []
        for idx, row in block[block["Configuration"] == "per_layer"].iterrows():
            row_ci = ConfidenceInterval(
                mean=row["micro_f1_mean"],
                std=0.0,
                ci_lower=row["micro_f1_ci_lower"],
                ci_upper=row["micro_f1_ci_upper"],
                n=10,
            )
...
def to_latex(df: pd.DataFrame, output_path: Path) -> None:
    """Render the results table as a LaTeX ``tabular`` and write it to ``output_path``."""
    lines = [
    ...
    for sigma_idx, sigma in enumerate(sorted(df["sigma"].unique())):
        ...
        for _, row in block.iterrows():
            ...
            delta_cell = (
                ...
                else f"{row['delta_vs_baseline']:+.4f}"
            )
            p_cell = (
                "—" if pd.isna(row["p_bonferroni"]) else f"{row['p_bonferroni']:.4f}"
            )
            config_label = (
                "Baseline (global clip)"
    ...
    lines.append(f"FL, no DP$^\\dagger$ & — & {FL_NODP_F1:.3f} & — & — & — & — \\\\")
...
def main() -> None:
    """CLI entry point: write the results table as CSV and LaTeX."""
    exp_root = Path(__file__).resolve().parent.parent
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--ci-csv", type=Path, default=exp_root / "logs" / "bootstrap_ci.csv"
    )
    parser.add_argument("--logs-dir", type=Path, default=exp_root / "logs")
    parser.add_argument(
        "--output-csv", type=Path, default=exp_root / "analysis" / "table_results.csv"
    )
    parser.add_argument(
        "--output-tex", type=Path, default=exp_root / "analysis" / "table_results.tex"
    )
