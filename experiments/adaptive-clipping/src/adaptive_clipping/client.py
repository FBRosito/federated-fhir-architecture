...
from adaptive_clipping.training import (
    adaptive_train_bert_one_round,
    adaptive_train_one_round,
)
...
_ADAPTIVE_CLIPPING_STRATEGY = os.getenv(
    "ADAPTIVE_CLIPPING_STRATEGY", "baseline"
).lower()
...
class AdaptiveClippingClient(FHIRFederatedClient):
    ...
    def fit(
        ...
        """Run one local training round and report clip norms/thresholds in the metrics.
            ...
            with GPUTimer() as _timer:
                if self._backend == "bert":
                    ...
                    if self.clipping_strategy == "per_layer":
                        ...
                        updated_params, n_examples, metrics = (
                            adaptive_train_bert_one_round(
                                model=self._model,
        ...
        if "grad_norms_pre_clip" in metrics:
            ...
            GradNormLogger().log_round(
                ...
                clip_thresholds=(
                    _json.loads(metrics.pop("clip_threshold"))
                    if self.clipping_strategy == "per_layer"
                    else metrics.pop("clip_threshold", None)
                ),
...
def client_fn(context: Context) -> fl.client.Client:
    """ClientApp factory: build this silo's client from run/node config."""
    ...
    client = AdaptiveClippingClient(
        ...
        clipping_strategy=str(
            run_cfg.get("clipping_strategy", _ADAPTIVE_CLIPPING_STRATEGY)
        ),
