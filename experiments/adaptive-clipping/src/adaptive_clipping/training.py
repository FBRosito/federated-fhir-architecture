...
def _normalized_thresholds(
    clipper: PerLayerClipper, max_grad_norm: float
) -> dict[str, float]:
        ...
        for step, batch in enumerate(dataloader):
            ...
            if accum_count % effective_accum_steps == 0:
                ...
                normalized_thresholds = _normalized_thresholds(
                    clipper, train_cfg.max_grad_norm
                )
...
def adaptive_train_one_round(
    ...
    if len(dataset) == 0:
        ...
        return (
            get_lora_parameters(model),
            0,
            {"loss": float("nan"), "perplexity": float("nan")},
        )
    n_total = max(len(dataset), 1)
    ...
    log.info(
        "Per-layer round completed — loss=%.4f | perplexity=%.2f", avg_loss, perplexity
    )
